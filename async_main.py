...
def get_image_from_DALL_E_3_API(user_prompt, image_dimension="1792x1024", image_quality="hd", model="dall-e-3", nb_final_image=1):
    ...
    if response.status_code == 200:
        ...
        file_path = f"images/img_{user_prompt[:50]}.png"  # Limiting prompt length to avoid too long file names
...
def get_video_from_Replicate_API(image_path, video_length="25_frames_with_svd_xt"):
    ...
    download.download_video_from_url(url, f"clips/video_{image_path[7:-4]}.mp4")
...
def main():
    ...
    if "clips" in story_json_dict:
        for clip_key, clip_value in story_json_dict["clips"].items():
            if "image_prompt" in clip_value:
                prompts.append(clip_value["image_prompt"])
    ...
    for prompt in prompts:
        ...
        video_paths.append(f"clips/video_img_{prompt[:50]}.mp4")
